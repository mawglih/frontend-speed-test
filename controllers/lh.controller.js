const lhService = require('../services/lh.service');
...
exports.postData = async function (req, res, next) {
  ...
  if(dest_URL == '') {
    ...
    try {
      let result = await lhService.getData(dest_URL);
      return res.status(200).json({ status: 200, data: result, message: 'LighHouse Data available'});
    } catch(e) {
